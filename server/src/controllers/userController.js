...
          // NOTA: Aquí asumo que getUserProfile devuelve el payload LIGERO para el doble fetch
...
          const newAccessToken = UserService.generateAccessToken({ id: decodedRefresh.id, rol: user.rol });
          ...
          // CREACIÓN - Usamos la configuración de seguridad + maxAge
          res.cookie('accessToken', newAccessToken, { ...cookieSecurityConfig, maxAge: ACCESS_TOKEN_MAX_AGE });

          return res.status(200).json(user);
        } catch {
          // BORRADO - Solo usamos la configuración de seguridad para asegurar SameSite y Path
...
  // ===============================
  logout: (req, res) => {
      // BORRADO - Solo usamos la configuración de seguridad
      res.clearCookie('accessToken', cookieSecurityConfig);
      res.clearCookie('refreshToken', cookieSecurityConfig);
