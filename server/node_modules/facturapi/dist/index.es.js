class ue {
   ...
   */
  update(t, n, c = null) {
    return this.client.put("/customers/" + t, { body: n, params: c });
  }
  /**
...
class fe {
   ...
   */
  previewPdf(t) {
    return this.client.post("/invoices/preview/pdf", { body: t });
  }
}
var de = /* @__PURE__ */ function() {
  return typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : window;
}(), { FormData: j, Blob: je, File: he } = de;
const Ee = "https://www.facturapi.io/v2", pe = "https://www.facturapi.io/v1", H = "v2", T = typeof process < "u" && !!process?.versions?.node, z = typeof navigator < "u" && navigator?.product === "ReactNative";
function _e(e) {
  return new Promise((t, n) => {
    const c = [];
    e.on("data", (d) => c.push(d)), e.on("end", () => t(Buffer.concat(c))), e.on("error", n);
  });
}
const B = async (e, t, n) => {
  if (T) {
    const c = e instanceof Buffer ? e : await _e(e);
    return new he([c], t, {
      type: n
    });
  }
...
class Oe {
  ...
  async uploadLogo(t, n) {
    ...
    return d.append("file", c, "file"), this.client.put("/organizations/" + t + "/logo", { formData: d });
  }
  /**
   ...
   * @returns Organization object
   */
  async uploadCertificate(t, n, c, d) {
    let a = new j();
    const [l, f] = await Promise.all([
      B(n, "cer.cer", "application/octet-stream"),
      B(c, "key.key", "application/octet-stream")
    ]);
    return a.append("cer", l, "cer.cer"), a.append("key", f, "key.key"), a.append("password", d), this.client.put("/organizations/" + t + "/certificate", {
      formData: a
    });
  }
  /**
...
class Ie {
  ...
  async validateSignature(t) {
    const { secret: n, signature: c, payload: d } = t;
    let a;
    if (typeof d == "string")
      a = d;
    ...
    if (T) {
      const l = await import("crypto"), h = l.createHmac("sha256", n).update(a).digest(), A = Buffer.from(c, "hex");
      ...
      const l = new TextEncoder(), f = l.encode(a), h = l.encode(n), A = await crypto.subtle.sign(
        "HMAC",
        ...
        f
      ), S = Array.from(new Uint8Array(A)).map((R) => R.toString(16).padStart(2, "0")).join("").toLowerCase();
      if (c !== S)
        throw new Error("Invalid signature");
    }
    return JSON.parse(a);
...
function ve() {
  return F || (F = 1, function(e, t) {
    var n = typeof globalThis < "u" && globalThis || typeof self < "u" && self || typeof D < "u" && D, c = function() {
      function a() {
      ...
      (function(l) {
        ...
        if (h.arrayBuffer)
          var S = [
            ...
            "[object Float32Array]",
            "[object Float64Array]"
          ], R = ArrayBuffer.isView || function(r) {
        ...
        function re(r) {
          var i = new FileReader(), s = U(i), u = /charset=([A-Za-z0-9_-]+)/.exec(r.type), E = u ? u[1] : "utf-8";
        ...
        function oe(r) {
          ...
          return r.trim().split("&").forEach(function(s) {
            if (s) {
              var u = s.split("="), E = u.shift().replace(/\+/g, " "), o = u.join("=").replace(/\+/g, " ");
...
`) === 0 ? u.substr(1, u.length) : u;
        ...
        function L(r, i) {
          return new Promise(function(s, u) {
            ...
            if (o.open(E.method, m(E.url), !0), E.credentials === "include" ? o.withCredentials = !0 : E.credentials === "omit" && (o.withCredentials = !1), "responseType" in o && (h.blob ? o.responseType = "blob" : h.arrayBuffer && (o.responseType = "arraybuffer")), i && typeof i.headers == "object" && !(i.headers instanceof p || f.Headers && i.headers instanceof f.Headers)) {
              var V = [];
              Object.getOwnPropertyNames(i.headers).forEach(function(_) {
                V.push(y(_)), o.setRequestHeader(_, N(i.headers[_]));
              }), E.headers.forEach(function(_, I) {
                V.indexOf(I) === -1 && o.setRequestHeader(I, _);
              });
            } else
              E.headers.forEach(function(_, I) {
                o.setRequestHeader(I, _);
              });
            E.signal && (E.signal.addEventListener("abort", g), o.onreadystatechange = function() {
              o.readyState === 4 && E.signal.removeEventListener("abort", g);
            }), o.send(typeof E._bodyInit > "u" ? null : E._bodyInit);
          });
        }
        return L.polyfill = !0, f.fetch || (f.fetch = L, f.Headers = p, f.Request = b, f.Response = O), l.Headers = p, l.Request = b, l.Response = O, l.fetch = L, l;
      })({});
...
var Pe = ve();
const Le = /* @__PURE__ */ Te(Pe);
let C;
T ? C = (e) => Buffer.from(e).toString("base64") : z ? C = (e) => globalThis.Buffer.from(e).toString("base64") : C = globalThis.btoa;
const me = async (e) => {
  if (!e.ok) {
    ...
    if (t.includes("image/") || t.includes("application/pdf") || t.includes("application/xml") || t.includes("application/zip"))
      if (T) {
        ...
        if (!n)
          return e.body;
        try {
          const { Readable: c } = await import("stream");
          return new c({
            read() {
...
const Ue = (e, t = H) => {
  ...
  return {
    ...
    async request(a, l) {
      const { params: f, body: h, formData: A, ...S } = l || {}, R = f ? "?" + new URLSearchParams(f).toString() : "", y = {
        ...S,
        headers: {
          ...c,
    ...
    get(a, l) {
      return this.request(a, { method: "GET", ...l });
    },
    post(a, l) {
      return this.request(a, { method: "POST", ...l });
    },
    put(a, l) {
      return this.request(a, { method: "PUT", ...l });
    },
    delete(a, l) {
      return this.request(a, { method: "DELETE", ...l });
    }
  };
};
var Me = /* @__PURE__ */ ((e) => (e.RECEIPT_SELF_INVOICE_COMPLETE = "receipt.self_invoice_complete", e.INVOICE_CANCELLATION_STATUS_UPDATED = "invoice.cancellation_status_updated", e.RECEIPT_STATUS_UPDATED = "receipt.status_updated", e.GLOBAL_INVOICE = "invoice.global_invoice_created", e.INVOICES_STATUS_UPDATED = "invoice.status_updated", e.INVOICES_CREATED_FROM_DASHBOARD = "invoice.created_from_dashboard", e.CUSTOMER_EDIT_LINK_COMPLETED = "customer.edit_link_completed", e))(Me || {}), Ge = /* @__PURE__ */ ((e) => (e.RECEIPT = "receipt", e.INVOICE = "invoice", e.CUSTOMER = "customer", e))(Ge || {}), Ve = /* @__PURE__ */ ((e) => (e.ENABLED = "enabled", e.DISABLED = "disabled", e))(Ve || {});
const k = ["v1", "v2"];
class ke {
  /**
  ...
  constructor(t, n = {}) {
    if (n.apiVersion) {
      ...
      this.apiVersion = H;
    this._wrapper = Ue(t, this.apiVersion), this.customers = new ue(this._wrapper), this.products = new ce(this._wrapper), this.invoices = new fe(this._wrapper), this.organizations = new Oe(this._wrapper), this.catalogs = new Ae(this._wrapper), this.receipts = new ye(this._wrapper), this.retentions = new be(this._wrapper), this.tools = new Se(this._wrapper), this.webhooks = new Ie(this._wrapper);
  }
}
export {
  Ge as ApiEventDataType,
  Me as ApiEventType,
  ee as CancellationMotive,
  De as CancellationStatus,
  Re as GlobalInvoicePeriodicity,
  X as IepsMode,
