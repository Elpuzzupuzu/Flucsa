...
  const reviews = Math.floor((product.id * 7) % 50) + 10;
  ...
  // 🚩 FUNCIÓN MODIFICADA PARA COMPROBAR AUTENTICACIÓN (Favoritos)
  const handleToggleLike = (e) => {
    e.stopPropagation();
    e.preventDefault();
    
    if (!isAuthenticated) {
      notify('Debes iniciar sesión para añadir a favoritos. 🔒'); // Notificación
      return; 
    }
...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden transition-all duration-200 group relative flex flex-col h-full hover:shadow-lg">
          {/* Ya NO necesitamos el componente de notificación aquí, ya que ToastNotification.jsx se encarga globalmente */}
...
              onClick={handleToggleLike} // 👈 USANDO FUNCIÓN CON CHECK
...
            onClick={handleAddToCartClick} // 👈 USANDO FUNCIÓN CON CHECK
