...
const ProductCard = ({ product, viewMode = 'grid', onAddToCart }) => {
  ...
  const formatPrice = (price) => {
    const numeric = parseFloat(price); // convierte a número
    if (isNaN(numeric)) return price;   // si no se puede convertir, devuelve tal cual
  ...
  if (viewMode === 'grid') {
    return (
      ...
      >
        {/* Si tienes ProductQuickView, descomenta esto */}
        {/* {isHovered && (
          <ProductQuickView
            product={product}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            <div className="text-center">
              <span className="text-2xl font-bold text-gray-900 block">{formatPrice(product.price)}</span>
  ...
  if (viewMode === 'list') {
    return (
      <Link
        ...
        className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 group overflow-hidden block"
      >
        {/* Tarjeta modo lista */}
        <div className="p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            <div className="relative flex-shrink-0 group/image">
              ...
              <button
                ...
                onClick={(e) => { e.preventDefault(); handleToggleLike(e); }}
                className={`absolute top-1 right-1 p-1.5 rounded-full transition-all duration-200 ${isLiked ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-red-400'}`}
              >
