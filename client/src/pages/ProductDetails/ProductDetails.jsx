...
const ProductDetails = ({ onAddToCart }) => {
  const { id } = useParams();
  const products = useSelector((state) => state.products.items); // Redux state
  ...
  useEffect(() => {
    ...
    if (foundProduct) {
      // Mapear los campos al formato que espera el JSX
      const mappedProduct = {
        ...foundProduct,
        name: foundProduct.nombre ?? 'Sin nombre',
        image: foundProduct.imagen ?? '',
        price: foundProduct.precio
          ? new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(foundProduct.precio)
          : 'Precio no disponible',
        description: foundProduct.descripcion ?? 'Sin descripción disponible',
        rating: foundProduct.rating ?? 4.5,
        reviews: foundProduct.reviews ?? 20,
      };
            ...
            <div className="space-y-6">
              <div className="animate-fadeInUp" style={{ animationDelay: '0.2s' }}>
                ...
                <div className="flex items-center space-x-4 mb-3">
                  <p className="text-2xl text-blue-600 font-bold animate-pulse">{product.price}</p>
                </div>
              </div>
