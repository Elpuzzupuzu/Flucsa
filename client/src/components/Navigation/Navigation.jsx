...
const Navigation = ({ isMobile }) => {
  ...
  return (
    <nav className={isMobile ? "flex flex-col space-y-4" : "hidden lg:flex items-center space-x-6"}>
      {/* Items de navegación - Usa Link para navegar */}
      <Link to="/" className={`${commonClasses} ${isMobile ? mobileClasses : desktopClasses}`}>
        Inicio
        {!isMobile && <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white group-hover:w-full transition-all duration-300"></span>}
      </Link>
      <Link to="/productos" className={`${commonClasses} ${isMobile ? mobileClasses : desktopClasses}`}>
        Productos
        {!isMobile && <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white group-hover:w-full transition-all duration-300"></span>}
      </Link>
      <Link to="/servicios" className={`${commonClasses} ${isMobile ? mobileClasses : desktopClasses}`}>
        Servicios
        {!isMobile && <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white group-hover:w-full transition-all duration-300"></span>}
      </Link>
      <Link to="/nosotros" className={`${commonClasses} ${isMobile ? mobileClasses : desktopClasses}`}>
        Nosotros
        {!isMobile && <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white group-hover:w-full transition-all duration-300"></span>}
      </Link>
      <Link to="/contacto" className={`${commonClasses} ${isMobile ? mobileClasses : desktopClasses}`}>
        Contacto
        {!isMobile && <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white group-hover:w-full transition-all duration-300"></span>}
      </Link>
